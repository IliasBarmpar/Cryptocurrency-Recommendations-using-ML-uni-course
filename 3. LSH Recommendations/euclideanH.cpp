/*
 * euclideanH.cpp
 *
 *  Created on: Oct 30, 2018
 *      Author: effy
 */
#include "euclideanH.hpp"

Rfamily *_rfam = NULL; //todo dhlwsh sto cpp extern sto hpp

Rfamily::Rfamily(){
	for(int i = 0; i < D; ++i){
		int temp = rand();
		temp = temp % 200;
		_rfamily.push_back(temp);
	}
}

//Creates an Hmember for the cosine family by taking D ammount of normally distributed numbers plus a uniform real number
euclideanH::euclideanH(){
	//_v = new VectorV;
	unsigned seed = chrono::system_clock::now().time_since_epoch().count();
	default_random_engine generator(seed);
	normal_distribution<double> distribution(0.0,1.0);

	for(int i=0; i<D; i++){
		double temp = distribution(generator);
		_v.push_back(temp);
	}

	//_t = new ShiftT;
	uniform_real_distribution<> dis(0.0,W);
	do{
		_t = dis(generator);
	}while(_t==W); // this addition ensures that we get a T in [0,w)!
}

// Calculates the value using the v's and the t when it is given a vector
int euclideanH::calculateValue(vector<double> *_p){ //calculate value given a point
	//cout << "initiating eclideanH->calculateValue EUCLIDEAN" << endl;
	double tempd = 0;
	vector< double >::iterator vecV_it = _v.begin();
	for(vector< double >::iterator vecP_it = _p->begin(); vecP_it != _p->end(); ++vecP_it, ++vecV_it){
		tempd += (*vecV_it)*(*vecP_it);	//v*p
	//	cout << "tempd=" << tempd <<endl;
	}
	tempd += _t; 		// v*p + t
	tempd = tempd / W;	//(v*p + t)/w
	tempd = tempd*100; //TODO VERY VERY experimental
	int temp = tempd;	//keep only the integer part
	//cout << "Finaltempd=" << tempd << endl;
	
	//char b = getchar();
	return temp;
}

euclideanFamily::euclideanFamily(int K){
	TableSize = 2000; //temp
	if(_rfam == NULL)
		_rfam = new Rfamily();
	
	for(int i = 0; i < K; i++){ //TODO random if needs be.
		euclideanH *temp = new euclideanH();
		hfam.push_back(*temp);
	}
}

void euclideanFamily::setPointer(Hfamily *ptr){
	ptr = this;
	return;
}

// simply returns the family of H members
void euclideanFamily::getValuesOfH(vector<double> *_p, vector<int> *hvalues){
	for(vector<euclideanH>::iterator hfam_it = hfam.begin(); hfam_it != hfam.end(); ++hfam_it)
		hvalues->push_back( (*hfam_it).calculateValue(_p) );
}

int euclideanFamily::getMapSize(int size){
	TableSize = size/32;
	return TableSize;
}

double euclideanDistOfTwoVectors(vector< double > *_q, vector< double > *_p){
	long double temp = 0;
	vector<double>::iterator p_it = _p->begin();
	for( vector<double>::iterator q_it = _q->begin(); q_it != _q->end(); ++q_it, ++p_it)
		temp += ( (*q_it) - (*p_it) )*( (*q_it) - (*p_it) );
	temp = sqrt( temp );
	return (double)temp;
}

double euclideanFamily::getDistance(vector< double > *_q, vector< double > *_p){
	//Check if they've got the same g functions
	/*
	int hflag = 0;
	cout << "hfamSize.size()=" << hfam.size() << endl;
	for(vector<euclideanH>::iterator h_it = hfam.begin(); h_it != hfam.end(); ++h_it){
		int temp1 = (*h_it).calculateValue(_q);
		int temp2 = (*h_it).calculateValue(_p);
		cout << "(*b_it)=" << temp1 << " | " << "(*c_it)=" << temp2 << endl;
		if( temp1 != temp2 ){
			hflag = 1;
			//break;
		}
	}
	if(hflag==1){
		return numeric_limits<double>::max();
	}else{
	}
	*/
	//Euclidean Distance
	return euclideanDistOfTwoVectors(_q, _p);
}
