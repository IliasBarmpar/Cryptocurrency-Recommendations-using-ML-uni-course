...
//	cout << "Assign + Update completed." << endl;
	auto end = std::chrono::steady_clock::now();
	duration = std::chrono::duration<double>(end-start).count(); //Computes in seconds.

//	for(int i = 0; i < clustAssigned.size(); ++i)
//		cout << clustAssigned[i] << ' ';
//	cout << endl;

//	for(int i = 0; i < kClusters; ++i)
//		cout <<  "vCounterPerCluster[" << i << "]=" << vCountPerCluster[i] << endl;
...
void myKmeans::assignment_Lloyds(){
	...
	int i = 0;
	for(vIter vc = vectors->begin(); vc != vectors->end(); ++vc, ++i )
		clustAssigned[i] = findLocationOfClosestCentroid( &(*vc));
}
...
void myKmeans::update_kmeans(){
	...
	int i = 0;
	for(vIter vc = vectors->begin(); vc != vectors->end(); ++vc, ++i){
		vCountPerCluster[clustAssigned[i]]++; //This (clustAssigned[vc->id]) cluster has vCount[] vectors

		int j = 0;
		for(vector<double>::iterator p_it = (vc->coordinates).begin(); p_it != (vc->coordinates).end(); ++j, ++p_it)
			centroids[clustAssigned[i]]->coordinates[j] = centroids[clustAssigned[i]]->coordinates[j] + (*p_it);
	}
...
void myKmeans::silhouette(){
	...
	int jj = 0;
	for(vc = vectors->begin(); vc != vectors->end(); ++vc, ++jj){
		//Initializations
...
//		cout << vc->id-1 << "| " << jj  << "| " << vectors->size() << endl;
		clustAssigned[vc->id-1] = temploc;
		vCountPerCluster[clustAssigned[vc->id-1]]++;
	...
	for(vc = vectors->begin(); vc != vectors->end(); ++vc){
		...
		if(vCountPerCluster[clustAssigned[vc->id-1]]==1){
			silAi[vc->id-1] = silBi[vc->id-1];
		}else{
...
//		vCountPerCluster[clustAssigned[n]]++;
	...
	for(int i = 0; i < kClusters; i++){
		...
		finalSi += clustSi[i];
		if(vCountPerCluster[i]==1)
			clNum--;
	}
	finalSi = finalSi / clNum;
	cout << "5" << endl; getchar();
		cout << finalSi << endl;
}
